...
class Wukong(object):
    def __init__(self, observation_w, observation_h, action_dim):
        ...
        self.low_hp_threshold = 60  # 低血阈值
        self.low_hp_cooldown = 0  
        self.death_reset = DeathResetController(  # 死亡reset控制器
            obs_window=self.obs_window,
            teleport_key="l",
            lock_on_key="0",
        )
        self.hp_count = 0  # 本局喝血次数
        self.hp_limit = 6
        self.heal_success_delta = 10      # 认为“喝成功”至少回血这么多像素
    ...
    def get_reward(self, boss_blood, next_boss_blood, self_blood, next_self_blood,
               ...
               stop, emergence_break, action, boss_attack, really_dead):
        print(next_self_blood, boss_blood)

        if really_dead:
            print("dead")
            print("true death detected, end episode")
            reward = -12
            done = 1
            stop = 0
            emergence_break += 1
        ...
        if  next_self_blood < self.low_hp_threshold:  # 血量为0的时候不喝血
            reward = -1
            done = 0
            stop = 0
            return reward, done, stop, emergence_break
            ...
            if next_self_blood - self_blood < -5:  # 如果掉血
                self_blood_reward = (next_self_blood - self_blood) // 5  # 惩罚，掉血越多惩罚越大
                print("掉血惩罚")
                time.sleep(0.05)  
                # 防止连续取帧时一直计算掉血
            ...
            if (action == 1 or action == 3) and boss_attack == True and next_self_stamina - self_stamina >= 7 and next_self_blood-self_blood == 0:
                print("完美闪避奖励")
                self_stamina_reward += 4
            elif (action == 1 or action == 3) and boss_attack == True and next_self_blood-self_blood == 0:
                print("成功闪避")
                self_stamina_reward += 1.5
            reward = reward + self_blood_reward * 2 + \
                boss_blood_reward * 1 + self_stamina_reward * 2  # 最终加权，boss掉血奖励权重大一些，闪避奖励权重小一些
            done = 0  # 还没有死
            emergence_break = 0 
            return reward, done, stop, emergence_break
    ...
    def step(self, action, boss_attack):
        ...
        really_dead = self.death_reset.is_really_dead(obs_screen, next_self_blood, next_boss_blood)  # 判断是否真的死了，返回布尔值

                # heal decision moved out of get_reward: keep reward function side-effect free
        if self.low_hp_cooldown > 0:
            self.low_hp_cooldown -= 1

        should_heal = (
            (not really_dead)
            and next_self_blood < self.low_hp_threshold
            and self.low_hp_cooldown == 0
            and self.hp_count < self.hp_limit  # 每局最多喝5次，防止过度依赖喝药
            #and (not boss_attack)      # boss在攻击时不喝，减少被打断
        )
        ...
        reward, done, stop, emergence_break = self.get_reward(self.boss_blood, next_boss_blood, self.self_blood, next_self_blood,
                                                              ...
                                                              self.stop, self.emergence_break, action, boss_attack,really_dead)
    ...
    def reset(self, initial=False): 
        self.death_reset.restart_after_death(initial)
        self.self_blood = 0
        self.boss_blood = 0
        self.self_stamina = 0
        self.boss_stamina = 0
        self.hp_count = 0
        #self.low_hp_cooldown = 0
        self.low_hp_cooldown = 0 # 重置环境，死了就重置，初始状态不需要重置
