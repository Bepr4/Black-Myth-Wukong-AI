...
FRAME_HISTORY_LEN = 1     # 这里没有做多帧堆叠，所以observation就是当前帧图像，历史帧长度为1
...
def Wukong_learn(env, double_dqn,checkpoint):  # 定义训练函数
...
def main():
    ...
    parser.add_argument("--double-dqn", type=int, default=1, help="double dqn - 0 = No, 1 = Yes") # 是否使用Double DQN
    ...
    env = Wukong(observation_w=175, observation_h=200, action_dim=4)  # 创建环境实例，输入参数为 观察空间的宽度、高度和动作空间的维度
